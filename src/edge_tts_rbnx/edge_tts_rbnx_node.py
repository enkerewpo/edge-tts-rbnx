...
class EdgeTTSNode(Node):
    def __init__(self):
        ...
        self.voice = self.get_parameter(
            'voice').get_parameter_value().string_value
        self.volume = self.get_parameter(
            'volume').get_parameter_value().string_value
    ...
    def text_callback(self, msg):
        ...
        self.logger.info(
            f"[prm] Received text on speech/tts/text, len={len(text)}, preview='{preview}'")
        if not text:
            self.logger.warning(
                "[prm] Empty text received, publishing status=False")
        ...
        try:
            ...
            self.logger.info(
                "[prm] TTS finished successfully, publishing status=True")
    ...
    async def speak_text(self, text: str):
        ...
        self.logger.info(
            f"[prm] Starting TTS: text_len={len(text)}, voice={self.voice}, volume={self.volume}")
        ...
        self.logger.info(
            f"[prm] Received {len(audio_bytes)} bytes of audio data")
        ...
        try:
            ...
            self.logger.info(
                f"Audio duration: {len(audio)}ms ({len(audio)/1000:.2f}s)")
        except Exception as e:
    ...
    def _configure_pulseaudio(self):
        ...
        try:
            ...
            if self.device:
                ...
                self.logger.info(
                    f"[prm] PulseAudio sink {self.device} configured: volume={self.volume}")
        except Exception as e:
    ...
    def _play_audio(self, audio_path: str):
        ...
        try:
            if self.device and ',' in self.device:
                ...
                self.logger.info(
                    f"Audio played successfully with aplay (device {device_str})")
            else:
                subprocess.run(
                    ["aplay", audio_path],
                    check=True, timeout=30,
                ...
                self.logger.info(
                    "Audio played successfully with aplay (default device)")
